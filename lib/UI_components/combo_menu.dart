...
class _DropdownMenuItemButtonState<T> extends State<_DropdownMenuItemButton<T>> {
  ...
  void _handleOnTap() {
    final ComboBoxItem<T> dropdownMenuItem = widget.route.items[widget.itemIndex].item!;
...
class ComboBoxButton<T> extends StatefulWidget {
...
class _DropdownButtonState<T> extends State<ComboBoxButton<T>> with WidgetsBindingObserver {
  int? _selectedIndex;
  ...
  @override
  void didUpdateWidget(ComboBoxButton<T> oldWidget) {
  ...
  void _updateSelectedIndex() {
    ...
    assert(widget.items!.where((ComboBoxItem<T> item) => item.value == widget.value).length == 1);
